...
def verfication_Field(data, test, msg=None):
    ...
    如果为字符串型，则校验字符串为非空，如果为整型，则校验字符串>=0
    ...
    if isinstance(data, dict):
        ...
        logging.info("---基本信息获取---")
        logging.info("传入的data为dict字典对象!")
        logging.info(('传入过来的dict对象主key的长度：%s' % len(data)))
        keys = data.keys()
        # logging.info(keys)

        logging.info('---开始进行字段校验---')
        ...
        for key, value in data.items():
            times = times + 1
            logging.info("")
            logging.info(("---传入的data数据，第%s对象元素,key值对应为:%s---" %
                          (times, key)))
            logging.info(('%s:%s' % (key, value)))
            analysis_subItem(value, msg=msg)

    elif isinstance(data, list):
        logging.info("---基本信息获取---")
        logging.info("传入的data为list列表对象!")
        logging.info(('传入过来的list列表对象的长度：%s' % len(data)))
        ...
        for i in range(data_list_len):
            # logging.info("")
            logging.info(("--传入的data数据，列表中第%s个子元素" % (i + 1)))
...
def listContain(list1, list2):
    error = []
    logging.info(("指定检测字段个数为:%s" % len(list1)))
    logging.info(("指定检测字段分别为：%s" % list1))
    logging.info(("实际获取到的字段个数为:%s" % len(list2)))
    logging.info(("实际获取到的字段分别为:%s" % list2))

    for i in list1:
        if i in list2:
            logging.info(
                ("被检测的字段【%s】在实际返回的列表字段:%s中" % (i, list2)))
            continue
        else:
            error.append(i)
    if error:
        for i in range(len(error)):
            logging.info(("请注意,被检测的字段【%s】不在实际返回的列表字段:%s中,请检查!" %
                          (error[i], list2)))
        raise RuntimeError(("在实际返回中，不存在的字段为:%s" % error))
...
def analysis_subItem(item, test=None, msg=None):
    '''
    ...
    if isinstance(item, str):
        ...
        logging.info(
            ('%s对应值的类型为list且长度为%s' % (item, value_list_len)))
        ...
        logging.info(
            ('%s对应值的类型为dict且长度为%s' % (item, value_dict_len)))
        subkey = item.keys()
        ...
        for subkey, subvalue in item.items():
            times = times + 1
            logging.info("")
            logging.info(("---内嵌的字典中,第%s对象元素,key值对应为:%s---" %
                          (times, subkey)))
            logging.info(('%s:%s' % (subkey, subvalue)))

            analysis_subItem(subvalue, msg=msg)
