...
#     '''
def convertCode(dataobj,encodeingtype,datatypes=None):
    if datatypes in ["List","list","LIST",None]:
        try:
            dataobj1 = json.dumps(dataobj,ensure_ascii=False,encoding=encodeingtype)
            print  dataobj1
            print  type(dataobj1)
            print "完成转码".decode('UTF-8')
            dataobj = json.loads(dataobj1)
            print  type(dataobj)
        except UnicodeEncodeError:
            try:
                dataobj = json.dumps(dataobj, ensure_ascii=False, encoding=encodeingtype)
                print  "未完成转码".decode('UTF-8')
            except:
                pass
            ...
            if objlen<=0:
                ...
                for indexN in range(0,objlen):
                    ...
                    if type(srcobj_n)==dict:
                        ...
                        findValue = FindAndValue(srcobj_n,filter,"[,|}|)]",1)
                        if findValue==None:
                            filtersValue="00000"
                        else:
                    ...
                    if  eval(conditionx)==False:
                        ...
                        print  u"第%s轮的%d次scrobj处理后：%s"  %(i-1,indexN+1,srcobj)
                    else:
                        korder = korder + 1
                        print "列表取值序号korder增加1=", korder
                        print  "%s 符合条件"  %srcobj_n
                        print   "condition为",condition
                        print  u"第%s轮的%d次scrobj为%s"  %(i-1,indexN+1,srcobj)
...
def pyfilter(srcobjs,condition,spitstring=None):
    ...
    if type(srcobj)==list:
        ...
        print "切割前的srcobj为".decode("UTF-8"),srcobj
        print 'srclist列表为'.decode("UTF-8"), srclist
        resrobj = funfilter(srclist,condition)
    else:
        resrobj="请将原数据转为字符类型或列表类型".decode("UTF-8")
    print  "pyfilter结果为:".decode("UTF-8"),resrobj
    print  "pyfilter End ..."
    return  resrobj
