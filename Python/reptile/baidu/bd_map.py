...
"""
    定义细分窗口的数量，横向X * 纵向Y
"""
WindowSize = {'xNum': 10.0, 'yNum': 10.0}
...
def getSmallRect(bigRect, windowSize, windowIndex):
    ...
    """
    offset_x = (
        bigRect['right']['x'] - bigRect['left']['x']) / windowSize['xNum']
    offset_y = (
        bigRect['right']['y'] - bigRect['left']['y']) / windowSize['yNum']
    left_x = bigRect['left']['x'] + offset_x * (
        windowIndex % windowSize['xNum'])
    left_y = bigRect['left']['y'] + offset_y * (
        windowIndex // windowSize['yNum'])
    right_x = (left_x + offset_x)
    right_y = (left_y + offset_y)
    return str(left_y) + ',' + str(left_x) + ',' + str(right_y) + ',' + str(
        right_x)
...
def requestBaiduApi(keyWords, smallRect, baiduAk, index, fileKey):
    ...
    pageNum = 0
    logfile = open(
        "./log/" + fileKey + "-" + today + ".log", 'a+', encoding='utf-8')
    file = open(
        "./result/" + fileKey + "-" + today + ".txt", 'a+', encoding='utf-8')
    # print('-------------')
    # print(index)
    while True:
        try:
            ...
            if len(res['results']) == 0:
                logfile.writelines(
                    time.strftime("%Y%m%d%H%M%S") + " stop " + str(index) +
                    " " + smallRect + " " + str(pageNum) + '\n')
                break
            else:
            ...
            print("except")
            logfile.writelines(
                time.strftime("%Y%m%d%H%M%S") + " except " + str(index) + " " +
                smallRect + " " + str(pageNum) + '\n')
            break
