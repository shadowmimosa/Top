...
def draw_menu():
    win.addstr(0, 0,
               "Score: " + str(len(snake) - 2) + "      Press 'q' to quit",
               curses.color_pair(5))
        ...
        for pos in snake:  # Snake is the list of [y, x], so we swap them below
            if n == 0:
                win.addstr(pos[1], pos[0], "@",
                           curses.color_pair(ex_foodcolor))  # Draws head
            else:
                win.addstr(pos[1], pos[0], "#", curses.color_pair(
                    ex_foodcolor))  # Draws segment of the body
            n += 1
...
def is_food_collision():
    for pos in food:
        if pos == snake[0]:
            ...
            foodcolor = random.randint(1,
                                       6)  # Pick random color of the next food
            return True
    return False
...
def game_over():
    ...
    win.addstr(
        10, 20,
        "Game over! Your score is " + str(len(snake)) + "  Press 'q' to quit",
        curses.color_pair(1))
